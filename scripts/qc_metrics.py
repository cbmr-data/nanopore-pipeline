...
def main_stats(args: StatsArgs) -> NoReturn:
    ...
    with sys.stdout if args.output is None else args.output.open("wt") as handle:
        for sample, filenames in samples.items():
            ...
            json.dump(stats, handle, cls=CustomJSONEncoder)
            print(file=handle, flush=True)
...
def main(argv: list[str]) -> None:
    tap.Parser(StatsArgs).bind(main_stats).run(argv)
